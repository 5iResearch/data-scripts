"""
1-Year Uptrend Regression Channel Screener - Canadian (TSX) version.

Same methodology as generate_uptrend_channel_screener.py, applied to a
Canadian universe instead of S&P 500 / Nasdaq-100: find stocks that have been
in a clean, strong uptrend over the past year - a tight regression channel
(high R^2) combined with a strong actual 1-year return. Ranks the whole
universe by a blended "quality of trend" score and lists the single best
combo names first, regardless of where price currently sits inside the
channel.
...
Outperformance is measured against XIC.TO (S&P/TSX Capped Composite).

Universe: every symbol in data/tsx_universe.csv + every ticker in
data/cdn_1w_rev_est_screener.csv, all downloaded as ".TO" listings.
"""
...
OUTPUT_DIR = os.path.join(REPO_ROOT, "outputs", "uptrend-channel-screener-cdn")
...
BENCH_LABEL = "XIC"
LOOKBACK_PERIOD = "1y"
MIN_TRADING_DAYS = 225  # ~91% of the ~248 trading days in 1y on the TSX; excludes recent IPOs
...
def to_yf_ticker(raw_ticker):
    return raw_ticker.strip().upper().replace(".", "-") + ".TO"


def bare_ticker(yf_ticker):
    return yf_ticker[:-3] if yf_ticker.endswith(".TO") else yf_ticker


def load_universe():
    tsx_symbols = load_tsx_symbols()  # already ".TO"-suffixed
    rev_df = pd.read_csv(REV_SCREENER_PATH).dropna(subset=["Ticker"])
    rev_tickers = [to_yf_ticker(t) for t in rev_df["Ticker"].astype(str)]

    name_map = {to_yf_ticker(t): n for t, n in zip(rev_df["Ticker"].astype(str), rev_df["Name"].astype(str))}
    if os.path.exists(KOYFIN_CDN_PATH):
        koyfin_df = pd.read_csv(KOYFIN_CDN_PATH).dropna(subset=["Ticker"])
        name_map.update({
            to_yf_ticker(t): n
            for t, n in zip(koyfin_df["Ticker"].astype(str), koyfin_df["Name"].astype(str))
        })

    combined = set(tsx_symbols) | set(rev_tickers)
    normalized = sorted({t for t in combined if t and t.strip()})
...
def load_revision_map():
    ...
    df["ticker"] = df["ticker"].astype(str).map(to_yf_ticker)
    return df.set_index("ticker").to_dict("index")


def load_cap_map():
    """Ticker (.TO-suffixed) -> Market Cap ($M), for the cap-tier filter. Merges both
    source CSVs; the rev-screener values win on overlap since that export is the more
    current of the two. Names in neither fall back to "Unknown" in cap_tier()."""
    cap_map = {}
    for path in (KOYFIN_CDN_PATH, REV_SCREENER_PATH):
...
def load_sector_map():
    """Ticker (.TO-suffixed) -> (Sector, Industry) from the Koyfin CDN CSV export.
    Empty for tickers not present there, since sector/industry labels are a
    ...
    if not os.path.exists(KOYFIN_CDN_PATH):
        ...
        return {}
    df = pd.read_csv(KOYFIN_CDN_PATH).dropna(subset=["Ticker"])
    return {
        to_yf_ticker(str(row["Ticker"])): (row.get("Sector", "") or "", row.get("Industry", "") or "")
...
def score_ticker(close, bench_close):
    ...
    return {
        ...
        f"1Y_{BENCH_LABEL}_Return_%": bench_return_1y,
...
def build_channel_chart(row, company_name, revision_row=None, sector_industry=None):
    close = row["_close"]
    ticker_display = bare_ticker(row["Ticker"])
...
def build_summary_table(rows):
    ...
    df["Ticker"] = df["Ticker"].map(bare_ticker)
    cols = ["Ticker", "Cap", "Composite_Score", "R2", "1Y_Return_%", f"1Y_{BENCH_LABEL}_Return_%", "Outperformance_%", "Z_Score", "Annual_Trend_Return_%"]
    df = df[cols]
    ...
    for _, r in df.iterrows():
        html.append(
            ...
            f"<td>{r[f'1Y_{BENCH_LABEL}_Return_%']:.0f}%</td>"
...
def build_section(rows, name_map, revision_map, sector_map=None):
    ...
    for row in survivors:
        company_name = name_map.get(row["Ticker"], bare_ticker(row["Ticker"]))
...
def main():
    ...
    out_path = os.path.join(OUTPUT_DIR, "Uptrend_Channel_Screener_CDN_latest.html")
...
<head>
<meta charset="utf-8">
<title>1-Year Uptrend Channel Screener &mdash; Canada</title>
...
<body>
<header>
  <h1>1-Year Uptrend Channel Screener &mdash; Canada</h1>
  <div class="meta">Generated {date_str} &middot; Strongest 1-year regression-channel fit (R&sup2;) combined with strongest actual 1-year return &middot; Benchmark: XIC (S&amp;P/TSX Capped Composite) &middot; Universe: TSX universe + Cdn revenue-revision screener</div>
  {cap_control}
</header>
{body}
</body>
