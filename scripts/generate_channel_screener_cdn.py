"""
10-Year Regression Channel Screener — Canadian (TSX) version.

Same methodology as generate_channel_screener.py, applied to a Canadian
universe instead of S&P 500 / Nasdaq-100:
...
Two extra filters keep both lists honest:
  ...
  2. The stock must have actually outperformed XIC.TO (S&P/TSX Capped
     Composite) cumulatively over the same 10-year window.
...
BENCH_LABEL = "XIC"
LOOKBACK_PERIOD = "10y"
MIN_TRADING_DAYS = 2200  # ~95% of the ~2500 trading days in 10y on the TSX; excludes recent IPOs
...
def score_ticker(close, bench_close):
    ...
    return {
        ...
        f"10Y_{BENCH_LABEL}_Return_%": bench_return_10y,
...
def build_summary_table(rows):
    ...
    df["Ticker"] = df["Ticker"].str.replace(r"\.TO$", "", regex=True)
    cols = ["Ticker", "Cap", "R2", "Z_Score", "10Y_Return_%", f"10Y_{BENCH_LABEL}_Return_%", "Outperformance_%", "Annual_Trend_Return_%"]
    df = df[cols]
    ...
    for _, r in df.iterrows():
        html.append(
            ...
            f"<td>{r[f'10Y_{BENCH_LABEL}_Return_%']:.0f}%</td>"
...
def main():
    ...
    out_path = os.path.join(OUTPUT_DIR, "Channel_Screener_CDN_latest.html")
...
<head>
<meta charset="utf-8">
<title>10-Year Regression Channel Screener — Canada</title>
...
<body>
<header>
  <h1>10-Year Regression Channel Screener — Canada</h1>
  <div class="meta">Generated {date_str} &middot; Long-term uptrend, historical outperformer vs XIC (S&amp;P/TSX Capped Composite), currently at a channel extreme &middot; Universe: TSX universe + Cdn revenue-revision screener</div>
  {cap_control}
</header>
{body}
</body>
