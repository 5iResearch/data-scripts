...
def write_hardcoded_sheet(ws, table_df):
    start_row = 8
    for offset in range(MAX_TEMPLATE_ROWS):
        row = start_row + offset
        if offset < len(table_df):
    ...
    if len(table_df) > MAX_TEMPLATE_ROWS:
        print(
            f"  WARNING: {len(table_df)} tickers found but template only has "
            f"{MAX_TEMPLATE_ROWS} rows; truncating to the top {MAX_TEMPLATE_ROWS}."
        )
